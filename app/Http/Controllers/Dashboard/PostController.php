...
class PostController extends Controller
{
    public function index() {
        if (Auth::user()->role == 3) {
            $posts = Post::with([ "tags", "user"])->withCount(["comments"])->orderBy("id", "DESC")->paginate(20);
        } else {
            $posts = Post::with(["tags", "user"])->withCount(["comments"])->orderBy("id", "DESC")->where("user_id", Auth::id())->paginate(20);
        }
    ...
    public function create() {
        ...
        return view("dashboard.post.add", compact("tags"));
    }
    ...
    public function store(Request $request) {
        ...
        $post = Post::create([
            ...
            "thumbnail" => $imageName,
            "status" => Arr::has($validated, "status"),
    ...
    public function trashed() {
        if (Auth::user()->role == 3) {
            $posts = Post::onlyTrashed()->with([ "tags", "user"])->withCount(["comments" => function($q) {
                $q->withTrashed();
